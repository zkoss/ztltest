...
fixture`ZTL TEST - B80-ZK-2865TestCafe`
...
test("B80-ZK-2865TestCafe", async (t) => {
...
<zk>
	...
	await t
		.wait(5000)
		.click(
			Selector(() => jq(".z-treechildren:eq(0) .z-tree-icon:eq(0)")[0]),
		);
	await ztl.waitResponse(t);
	await t
		.expect(
			await ClientFunction(
				() => !!jq(".z-treechildren:eq(0) .z-treerow:eq(0) tr")[0],
			)(),
		)
		.notOk();
	await t.click(
		Selector(() => jq(".z-treechildren:eq(1) .z-tree-icon:eq(0)")[0]),
	);
	await ztl.waitResponse(t);
	await t
		.expect(
			await ClientFunction(
				() => !!jq(".z-treechildren:eq(1) .z-treerow:eq(0) tr")[0],
			)(),
		)
		.notOk();
	await t.click(
		Selector(() => jq(".z-treechildren:eq(2) .z-tree-icon:eq(0)")[0]),
	);
	await ztl.waitResponse(t);
	await t
		.expect(
			await ClientFunction(
				() => !!jq(".z-treechildren:eq(2) .z-treerow:eq(0) tr")[0],
			)(),
		)
		.notOk();
	await t.click(
		Selector(() => jq(".z-treechildren:eq(3) .z-tree-icon:eq(0)")[0]),
	);
	await ztl.waitResponse(t);
	await t
		.expect(
			await ClientFunction(
				() => !!jq(".z-treechildren:eq(3) .z-treerow:eq(0) tr")[0],
			)(),
		)
		.notOk();
	await t.click(
		Selector(() => jq(".z-treechildren:eq(4) .z-tree-icon:eq(0)")[0]),
	);
	await ztl.waitResponse(t);
	await t
		.expect(
			await ClientFunction(
				() => !!jq(".z-treechildren:eq(4) .z-treerow:eq(0) tr")[0],
			)(),
		)
		.notOk();
	await t.click(
		Selector(() => jq(".z-treechildren:eq(5) .z-tree-icon:eq(0)")[0]),
	);
	await ztl.waitResponse(t);
	await t
		.expect(
			await ClientFunction(
				() => !!jq(".z-treechildren:eq(5) .z-treerow:eq(0) tr")[0],
			)(),
		)
		.notOk();
});
