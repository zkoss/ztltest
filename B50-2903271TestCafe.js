...
fixture`ZTL TEST - B50-2903271TestCafe`
...
test("B50-2903271TestCafe", async (t) => {
...
and tab2 will be selected
	...
	await t.click(
		Selector(() => jq('@button[label="close 1 and select 5"]')[0]),
	);
	await ztl.waitResponse(t);
	await t
		.expect(
			await ClientFunction(() =>
				jq("$tab5").hasClass("z-tab-selected"),
			)(),
		)
		.ok();
	...
	await t
		.expect(
			await ClientFunction(() =>
				jq("$tab2").hasClass("z-tab-selected"),
			)(),
		)
		.ok();
});
