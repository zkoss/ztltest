...
"""
    ...
    runZTL(zul, () => {
      ...
      verifyEquals("1", jq("$radiogroup").toWidget().attr("selectedIndex"))
      ...
      verifyEquals("0", jq("$radiogroup").toWidget().attr("selectedIndex"))
      verifyTrue(jq("$radioa").toWidget().is("checked"))
      verifyFalse(jq("$radiob").toWidget().is("checked"))
      verifyFalse(jq("$radioc").toWidget().is("checked"))
      click(jq("$radiob").toWidget().$n("real"))
      waitResponse()
      verifyEquals("1", jq("$listbox").toWidget().attr("selectedIndex"))
      verifyEquals("1", jq("$radiogroup").toWidget().attr("selectedIndex"))
      ...
      verifyTrue(jq("$radiob").toWidget().is("checked"))
      verifyFalse(jq("$radioc").toWidget().is("checked"))
      click(jq("$radioc").toWidget().$n("real"))
      waitResponse()
      verifyEquals("2", jq("$listbox").toWidget().attr("selectedIndex"))
      verifyEquals("2", jq("$radiogroup").toWidget().attr("selectedIndex"))
      verifyFalse(jq("$radioa").toWidget().is("checked"))
      verifyFalse(jq("$radiob").toWidget().is("checked"))
