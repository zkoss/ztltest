...
"""
    ...
    runZTL(zul, () => {
      verifyEquals("1", jq("$listbox2").toWidget().attr("selectedIndex"))
      verifyEquals("1", jq("$tabbox2").toWidget().attr("selectedIndex"))
      ...
      click(jq("$item2a"))
      waitResponse()
      verifyEquals("0", jq("$listbox2").toWidget().attr("selectedIndex"))
      verifyEquals("0", jq("$tabbox2").toWidget().attr("selectedIndex"))
      verifyTrue(jq("$tab2a").toWidget().is("selected"))
      verifyFalse(jq("$tab2b").toWidget().is("selected"))
      verifyFalse(jq("$tab2c").toWidget().is("selected"))
      click(jq("$tab2c"))
      waitResponse()
      verifyEquals("2", jq("$listbox2").toWidget().attr("selectedIndex"))
      verifyEquals("2", jq("$tabbox2").toWidget().attr("selectedIndex"))
      verifyFalse(jq("$tab2a").toWidget().is("selected"))
      verifyFalse(jq("$tab2b").toWidget().is("selected"))
