...
"""
    ...
    for (i <- 0 to btns.length - 1) {
      runZTL(zul, () => {
    	  ...
    	  verifyEquals(answer(i)(0), comp1s.eq(0).toWidget().attr("label"))
    	  verifyEquals(answer(i)(1), comp1s.eq(1).toWidget().attr("label"))
    	  verifyEquals(answer(i)(2), comp1s.eq(2).toWidget().attr("label"))
    	  verifyEquals(answer(i)(3), comp1s.eq(3).toWidget().attr("label"))
    	  verifyEquals(answer(i)(4), comp1s.eq(4).toWidget().attr("label"))
    	  verifyEquals(answer(i)(5), comp1s.eq(5).toWidget().attr("label"))
    	  val box1s = jq("$box1 @label")
    	  verifyEquals(2, box1s.length)
    	  verifyEquals(answer(i)(6), box1s.eq(0).toWidget().attr("value"))
    	  verifyEquals(answer(i)(7), box1s.eq(1).toWidget().attr("value"))
      })
    }
