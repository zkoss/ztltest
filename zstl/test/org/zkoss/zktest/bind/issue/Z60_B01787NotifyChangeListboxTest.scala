/* Z60_B01787NotifyChangeListboxTest.scala
 ...
 */
@Tags(tags = "zbind")
class Z60_B01787NotifyChangeListboxTest extends ZTL4ScalaTestCase {

  def testArg() = {
    val zul = """
      <include src="/bind/issue/B01787NotifyChangeListbox.zul"/>
"""
    ...
    for (i <- 0 to btns.length - 1) {
      runZTL(zul, () => {
    	  ...
    	  val comp1s = jq("$listbox @listitem")
