...
"""
    ...
    runZTL(zul, () => {
      ...
      verifyEquals("1", jq("$tabbox").toWidget().attr("selectedIndex"))
      ...
      verifyEquals("0", jq("$tabbox").toWidget().attr("selectedIndex"))
      ...
      verifyEquals("2", jq("$tabbox").toWidget().attr("selectedIndex"))
      verifyFalse(jq("$taba").toWidget().is("selected"))
      verifyFalse(jq("$tabb").toWidget().is("selected"))
