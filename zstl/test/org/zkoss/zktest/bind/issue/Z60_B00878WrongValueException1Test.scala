...
"""
    ...
    runZTL(zul, () => {
		...
		var save = jq("$save")
		verifyEquals("", msgName.toWidget().attr("value"))
		verifyEquals("0", msgAge.toWidget().attr("value"))
		verifyEquals("0", msgScore.toWidget().attr("value"))
		...
		click(save.toWidget())
		waitResponse()
		verifyEquals("Lin", msgName.toWidget().attr("value"))
		verifyEquals("24", msgAge.toWidget().attr("value"))
		verifyEquals("34", msgScore.toWidget().attr("value"))
		errorPopup = jq(".z-errorbox")
