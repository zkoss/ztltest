/* Z60_B01787NotifyChangeComboboxTest.scala
 ...
 */
@Tags(tags = "zbind")
class Z60_B01787NotifyChangeComboboxTest extends ZTL4ScalaTestCase {

  def testArg() = {
    val zul = """
      <include src="/bind/issue/B01787NotifyChangeCombobox.zul"/>
"""
    ...
    for (i <- 0 to btns.length - 1) {
      runZTL(zul, () => {
    	  ...
    	  jq("$combobox").toWidget().eval("open()")
    	  waitResponse()
    	  val comp1s = jq("$combobox @comboitem")
