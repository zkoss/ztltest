...
class B36_2813877Test extends ZTL4ScalaTestCase {
  ...
  def testresizer() = {
    ...
    runZTL(zscript, () => {
      ...
      waitResponse()
      mouseDownAt(jq("$win"), "341,292")
      mouseMoveAt(jq("$win"), "619,298")
      mouseUpAt(jq("$win"), "618,298")
      mouseDownAt(jq("@caption"), "174,19")
      mouseMoveAt(jq("@caption"), "176,66")
      mouseUpAt(jq("@caption"), "176,66")
      click(jq("@button[label=\"Embed\"]"))
      waitResponse()
      click(jq("@button[label=\"Popup\"]"))
      waitResponse()
      mouseDownAt(jq("$win").toWidget().$n("cave"), "349,222")
      mouseMoveAt(jq("$win").toWidget().$n("cave"), "654,218")
      mouseUpAt(jq("$win").toWidget().$n("cave"), "654,218")
    })
  }
}
