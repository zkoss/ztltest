...
class B86_ZK_4011Test extends ZTL4ScalaTestCase {
  ...
  def test(): Unit =  {
    runZTL(() => {
      ...
      verifyTolerant(0, getWidth(jq("@cell:eq(0)")), 1)
      verifyTolerant(0, getWidth(jq("@cell:eq(3)")), 1)
      verifyTolerant(0, getWidth(jq("@cell:eq(5)")), 1)
      verifyTolerant(0, getWidth(jq("@auxheader:eq(2)")), 1)

      click(jq("@button:contains(switch 1 visible)"))
      waitResponse()
      verifyTrue(getWidth(jq("@cell:eq(0)")) > 1)

      click(jq("@button:contains(switch 6 visible)"))
      waitResponse()
      verifyTrue(getWidth(jq("@cell:eq(5)")) > 1)
