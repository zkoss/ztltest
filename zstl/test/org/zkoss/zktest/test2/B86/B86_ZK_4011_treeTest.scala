...
class B86_ZK_4011_treeTest extends ZTL4ScalaTestCase {
  ...
  def test(): Unit =  {
    runZTL(() => {
      ...
      verifyTolerant(0, getWidth(jq("@treecell:eq(0)")), 1)
      verifyTolerant(0, getWidth(jq("@treecell:eq(3)")), 1)
      verifyTolerant(0, getWidth(jq("@treecell:eq(5)")), 1)
      verifyTolerant(0, getWidth(jq("@auxheader:eq(2)")), 1)

      click(jq("@button:contains(switch 1 visible)"))
      waitResponse()
      verifyTrue(getWidth(jq("@treecell:eq(0)")) > 1)

      click(jq("@button:contains(switch 6 visible)"))
      waitResponse()
      verifyTrue(getWidth(jq("@treecell:eq(5)")) > 1)
