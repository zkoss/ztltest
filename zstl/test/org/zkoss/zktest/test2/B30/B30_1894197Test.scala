...
class B30_1894197Test extends ZTL4ScalaTestCase {
  def testClick() = {
    ...
    runZTL(zscript, () => {
      ...
      verifyContains("It should be visible the 'Node: A' node", jq(".z-treecell").text(), "Node: A");
      verifyContains("It should be visible the 'Node: B' node", jq(".z-treecell").text(), "Node: B");
    })
  }
}