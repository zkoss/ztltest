...
<zk>
    ...
    runZTL(zscript, () => {
      ...
      verifyTolerant(width1, jq("@listcell").eq(0).outerWidth(), 1)
      verifyTolerant(width1, jq("@listheader").eq(0).outerWidth(), 1)
      verifyTolerant(width2, jq("@listcell").eq(1).outerWidth(), 1)
      verifyTolerant(width2, jq("@listheader").eq(1).outerWidth(), 1)
      click(jq("@button"))
      waitResponse()
      verifyFalse(jq("@listhead").isVisible())
      verifyTolerant(width1, jq("@listcell").eq(0).outerWidth(), 1)
