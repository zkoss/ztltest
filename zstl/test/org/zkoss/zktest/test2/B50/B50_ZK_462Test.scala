...
class B50_ZK_462Test extends ZTL4ScalaTestCase {
  ...
  def testztl() = {
    ...
    runZTL(zscript, () => {
      click(btn1)
      waitResponse()
      verifyTrue("nothing happened".equals(lb.$n().attr("innerHTML")))
      click(cb1.$n("real"))
      waitResponse()
      click(cb1.$n("real"))
      waitResponse()
      ...
      click(cb4.$n("real"))
      waitResponse()
      verifyEquals("nothing happened", lb.$n().attr("innerHTML"))
      click(win.$n("max"))
      waitResponse()
      verifyEquals("oops, test win maximized!", lb.$n().attr("innerHTML"))
      click(btn1)
      waitResponse()
      verifyEquals("nothing happened", lb.$n().attr("innerHTML"))
      click(win.$n("max"))
