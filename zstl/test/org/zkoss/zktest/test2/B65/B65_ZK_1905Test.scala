...
<zk>
    ...
    runZTL(zscript,
      () => {
        ...
        click(inp)
        waitResponse()
        verifyNotContains("the '--------' text should be removed", inp.get("value"), "-")
        blur(inp)
        waitResponse()
        verifyContains("the '--------' text should appear again", inp.get("value"), "-")
        click(inp)
        waitResponse()
        ...
        waitResponse()
        verifyNotContains("the '--------' text should not appear", inp.get("value"), "-")
      })
