...
class B65_ZK_2004Test extends ZTL4ScalaTestCase {
  ...
  @Test
  def testClick() = {
    val zscript = """<window title="Chosebox model replacement" border="normal" width="350px">
	<zscript><![CDATA[
        int i = 0;
    ...
    runZTL(zscript,
      () => {
        ...
        verifyTrue("The chosenbox should not contain selected items.", !jq(".z-chosenbox-sel-item").exists)
      })
