...
"""
    runZTL(zscript,
      () => {
        val winC = jq(".z-window-overlapped-header:contains(C)")
        val winB = jq(".z-window-overlapped-header:contains(B)")

        val dragDrop = (comp: JQuery, posi: String, newPosi: String) => {
        ...
        dragDrop(winC, "2,2", "200,200")
        dragDrop(winB, "2,2", "200,200")
