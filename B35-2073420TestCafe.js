...
fixture`ZTL TEST - B35-2073420TestCafe`
...
test("B35-2073420TestCafe", async (t) => {
...
<group label="Date:Sunday 2" id="g5"/>
	...
	await t.expect(await ClientFunction(() => !!jq("$lbl1")[0])()).ok();
	await t.click(Selector(() => jq("$upd1")[0]));
	await ztl.waitResponse(t);
	await t.expect(await ClientFunction(() => !!jq("$lbl1")[0])()).notOk();
	let ex_cafe = await ClientFunction(() => !!jq("$img1")[0])();
	await t.expect(ex_cafe).ok();
	let ex1_cafe = await ClientFunction(() => !!jq("$g1").find("$img1")[0])();
	await t.expect(ex1_cafe).ok();
	await t.click(Selector(() => jq("$upd1")[0]));
	await ztl.waitResponse(t);
	await t
		.expect(await ClientFunction(() => !!jq("$g1").find("span")[0])())
		.ok();
});
