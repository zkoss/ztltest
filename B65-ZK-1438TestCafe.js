...
fixture`ZTL TEST - B65-ZK-1438TestCafe`
...
test("B65-ZK-1438TestCafe", async (t) => {
	...
	await t.click(Selector(() => jq("@button:contains(sendEvent)")[0]));
	...
	await t
		...
		.eql(
			ztl.normalizeText(
				await ClientFunction(
					() => jq("@listbox:eq(0) .z-listcell").length,
				)(),
			),
			"should see only one message shows in listbox",
		);
	await t
		.expect(ztl.normalizeText("1"))
		.eql(
			ztl.normalizeText(
				await ClientFunction(
					() => jq("@listbox:eq(1) .z-listcell").length,
				)(),
			),
			"should see only one message shows in listbox",
		);
	await t.click(Selector(() => jq("@button:contains(reload 1)")[0]));
	await ztl.waitResponse(t);
	await t.click(Selector(() => jq("@button:contains(sendEvent)")[0]));
	await ztl.waitResponse(t);
	await t
		...
		.eql(
			...
			"should see only one message shows in listbox1 and one in listbox2",
		);
	await t
		.expect(ztl.normalizeText("1"))
		.eql(
	...
	await t.click(Selector(() => jq("@button:contains(detach)")[0]));
	await ztl.waitResponse(t);
	await t.click(Selector(() => jq("@button:contains(sendEvent)")[0]));
	await ztl.waitResponse(t);
	await t
		...
		.eql(
			...
			"should see no message shows in listbox1 and 2",
		);
	await t
		.expect(ztl.normalizeText("0"))
		.eql(
