...
fixture`ZTL TEST - B30-1894197TestCafe`
...
test("B30-1894197TestCafe", async (t) => {
	...
	let count_cafe = await ClientFunction(() => jq(".z-treerow").length)();
	await t
		.expect(count_cafe == 1)
		.ok("It should be visible only the root node");
	await t.click(Selector(() => jq("@button")[0]));
	await ztl.waitResponse(t);
	let countAfter_cafe = await ClientFunction(() => jq(".z-treerow").length)();
	await t
		.expect(countAfter_cafe == 3)
		.ok("It should be visible the root and two child nodes (3)");
	await t
		.expect(
			ztl.normalizeText(
				await ClientFunction(() =>
					jq(".z-treecell").text().replace(/\s/g, " "),
				)(),
			),
		)
		.contains(
			ztl.normalizeText("Node: A"),
			"It should be visible the 'Node: A' node",
		);
	await t
		.expect(
			ztl.normalizeText(
		...
		.contains(
			ztl.normalizeText("Node: B"),
			"It should be visible the 'Node: B' node",
		);
});
