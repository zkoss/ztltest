...
fixture`ZTL TEST - B50-2940707-1TestCafe`
...
test("B50-2940707-1TestCafe", async (t) => {
	...
	await ztl.waitResponse(t);
	await t.click(Selector(() => jq(".z-treerow-checkbox:eq(3)")[0]));
	await ztl.waitResponse(t);
	await t.click(Selector(() => jq(".z-treerow-checkbox:eq(4)")[0]));
	await t
		.expect(
			ztl.normalizeText(
	...
	await t
		.expect(
			ztl.normalizeText(
				await ClientFunction(() =>
					jq(".z-treerow-selected:eq(0) .z-treecell")
						.text()
						.replace(/\s/g, " "),
				)(),
			),
		)
		.contains(ztl.normalizeText("Item 2.1"), "");
	await t
		.expect(
			ztl.normalizeText(
				await ClientFunction(() =>
					jq(".z-treerow-selected:eq(1) .z-treecell")
						.text()
						.replace(/\s/g, " "),
				)(),
			),
		)
		.contains(ztl.normalizeText("Item 2.1.1"), "");
	await t
		.expect(
			ztl.normalizeText(
				await ClientFunction(() =>
					jq(".z-treerow-selected:eq(2) .z-treecell")
						.text()
						.replace(/\s/g, " "),
				)(),
			),
		)
		.contains(ztl.normalizeText("Item 2.1.2"), "");
	await t.click(Selector(() => jq(".z-treerow-checkbox:eq(2)")[0]));
	await ztl.waitResponse(t);
	await t
		.expect(
