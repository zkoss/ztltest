...
fixture`ZTL TEST - Z60-CollectionIndexCombobox2TestCafe`
...
test("Z60-CollectionIndexCombobox2TestCafe", async (t) => {
	...
	await ClientFunction(() => {
		zk.Widget.$(
			jq("$outergrid").find("@rows").children().eq(4).find("@combobox"),
		).open();
	})();
	await ztl.waitResponse(t);
	await t
	...
	await t.click(
		Selector(() =>
			zk.Widget.$(
				jq("$outergrid").find("@rows").children().eq(4).find("@button"),
			).$n(),
		),
	);
	await ztl.waitResponse(t);
