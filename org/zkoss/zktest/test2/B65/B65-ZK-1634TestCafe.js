...
fixture`ZTL TEST - B65-ZK-1634TestCafe`
...
test("B65-ZK-1634TestCafe", async (t) => {
...
<zk>
	...
	await t
		.expect(
			await ClientFunction(
				() => !!jq(".z-treecell:contains(New Node 2)")[0],
			)(),
		)
		.ok("the node 2 label should change to New Node 2");
	await t
		.expect(
			await ClientFunction(
				() => !!jq(".z-treecell:contains(Node 3)")[0],
			)(),
		)
		.ok("still has child Node3");
	await t
		.expect(await ClientFunction(() => !!jq(".z-window-modal")[0])())
		.notOk("you should not see any exception dialog");
});
