...
fixture`ZTL TEST - B65-ZK-1639TestCafe`
...
test("B65-ZK-1639TestCafe", async (t) => {
	...
	await t.click(
		Selector(
			() => jq(".z-button:contains(Move Node 3 to Node X):eq(0)")[0],
		),
	);
	...
	await t
		.expect(
			await ClientFunction(
				() => !!jq(".z-treerow:eq(5):contains(Node 3)")[0],
			)(),
		)
		.ok("the node 3 should move to node X");
	await t
		.expect(
			await ClientFunction(
				() =>
					zk.Widget.$(jq(".z-treerow:eq(5):contains(Node 3)")).$n(
						"icon",
					) != null,
			)(),
		)
	...
	await t.click(
		Selector(
			() => jq(".z-button:contains(Move Node 3 to Node 2):eq(0)")[0],
		),
	);
	await ztl.waitResponse(t);
	await t
		.expect(
			await ClientFunction(
				() => !!jq(".z-treerow:eq(3):contains(Node 3)")[0],
			)(),
		)
		.ok("the node 3 should move to node 2");
	await t
		.expect(
			await ClientFunction(
				() =>
					zk.Widget.$(jq(".z-treerow:eq(3):contains(Node 3)")).$n(
						"icon",
					) != null,
			)(),
		)
	...
	await t.click(
		Selector(
			() => jq(".z-button:contains(Move Node 3 to Node X):eq(1)")[0],
		),
	);
	await ztl.waitResponse(t);
	await t
	...
	await t
		.expect(
			await ClientFunction(
				() => !!jq(".z-treerow:eq(1):contains(Node (1))")[0],
			)(),
		)
		.ok("both node2 and nodeX has correct label change");
	await t
		.expect(
			await ClientFunction(
				() => !!jq(".z-treerow:eq(3):contains(Node (2))")[0],
			)(),
		)
		.ok("both node2 and nodeX has correct label change");
	await t.click(
		Selector(
			() => jq(".z-button:contains(Move Node 3 to Node 2):eq(1)")[0],
		),
	);
	await ztl.waitResponse(t);
	await t
	...
	await t
		.expect(
			await ClientFunction(
				() => !!jq(".z-treerow:eq(1):contains(Node (2))")[0],
			)(),
		)
		.ok("both node2 and nodeX has correct label change");
	await t
		.expect(
			await ClientFunction(
				() => !!jq(".z-treerow:eq(4):contains(Node (1))")[0],
			)(),
		)
		.ok("both node2 and nodeX has correct label change");
});
