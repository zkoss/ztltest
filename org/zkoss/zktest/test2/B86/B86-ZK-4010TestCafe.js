...
fixture`ZTL TEST - B86-ZK-4010TestCafe`
	.page`http://localhost:8080/zktest/test2/B86-ZK-4010.zul`.beforeEach(
	async () => {
...
test("B86-ZK-4010TestCafe", async (t) => {
	...
	await ztl.verifyTolerant(
		...
		ztl.normalizeText("0"),
		ztl.normalizeText(
			await ClientFunction(() => jq("@listcell:eq(0)")[0].clientWidth)(),
		),
		ztl.normalizeText("1"),
	);
	await ztl.verifyTolerant(
		...
		ztl.normalizeText("0"),
		ztl.normalizeText(
			await ClientFunction(() => jq("@listcell:eq(3)")[0].clientWidth)(),
		),
		ztl.normalizeText("1"),
	);
	await ztl.verifyTolerant(
		...
		ztl.normalizeText("0"),
		ztl.normalizeText(
			await ClientFunction(() => jq("@listcell:eq(5)")[0].clientWidth)(),
		),
		ztl.normalizeText("1"),
	);
	await ztl.verifyTolerant(
