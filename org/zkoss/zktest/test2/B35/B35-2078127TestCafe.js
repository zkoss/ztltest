...
fixture`ZTL TEST - B35-2078127TestCafe`
...
test("B35-2078127TestCafe", async (t) => {
...
f1.detach();
	...
	await t
		.expect(
			await ClientFunction(() =>
				jq("@listgroup:contains(1)")
					.find("i")
					.hasClass("z-listgroup-icon-open"),
			)(),
		)
		.ok("The '+' icon should be visible");
	let css1Before_cafe = await ClientFunction(() =>
		jq(zk.Widget.$(jq(".z-listcell")).$n("cave")).css("font-weight"),
	)();
	let css2Before_cafe = await ClientFunction(() =>
		jq(zk.Widget.$(jq(".z-listcell")).$n("cave")).css("font-size"),
	)();
	let css3Before_cafe = await ClientFunction(() =>
		jq(zk.Widget.$(jq(".z-listcell")).$n("cave")).css("font-family"),
	)();
	let css4Before_cafe = await ClientFunction(() =>
		jq(zk.Widget.$(jq(".z-listcell")).$n("cave")).css("font-color"),
	)();
	await t.click(Selector(() => jq("@button")[1]));
	await ztl.waitResponse(t);
	await t.click(Selector(() => jq("@button")[1]));
	await ztl.waitResponse(t);
	let css1After_cafe = await ClientFunction(() =>
		jq(zk.Widget.$(jq(".z-listcell")).$n("cave")).css("font-weight"),
	)();
	let css2After_cafe = await ClientFunction(() =>
		jq(zk.Widget.$(jq(".z-listcell")).$n("cave")).css("font-size"),
	)();
	let css3After_cafe = await ClientFunction(() =>
		jq(zk.Widget.$(jq(".z-listcell")).$n("cave")).css("font-family"),
	)();
	let css4After_cafe = await ClientFunction(() =>
		jq(zk.Widget.$(jq(".z-listcell")).$n("cave")).css("font-color"),
	)();
	await t
		.expect(ztl.normalizeText(css1After_cafe))
		.eql(
			ztl.normalizeText(css1Before_cafe),
			"The font-weight should be equal than before",
		);
	await t
		.expect(ztl.normalizeText(css2After_cafe))
		.eql(
			ztl.normalizeText(css2Before_cafe),
			"The font-size should be equal than before",
		);
	await t
		.expect(ztl.normalizeText(css3After_cafe))
		.eql(
			ztl.normalizeText(css3Before_cafe),
			"The font-family should be equal than before",
		);
	await t
		.expect(ztl.normalizeText(css4After_cafe))
		.eql(
			ztl.normalizeText(css4Before_cafe),
			"The font-color should be equal than before",
		);
});
