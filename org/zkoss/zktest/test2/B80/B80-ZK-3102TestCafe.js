...
fixture`ZTL TEST - B80-ZK-3102TestCafe`
	.page`http://localhost:8080/zktest/test2/B80-ZK-3102.zul`.beforeEach(
	async () => {
...
test("B80-ZK-3102TestCafe", async (t) => {
	await ztl.initTest(t);
	if (
		await ClientFunction(
			() =>
				jq(jq("@combobox").find(".z-combobox-input"))[0] !=
				document.activeElement,
		)()
	)
		await t.click(
			Selector(() => jq("@combobox").find(".z-combobox-input")[0]),
		);
	...
	await ztl.verifyTolerant(
		t,
		ztl.normalizeText(verifyVariable_cafe_8_8),
		ztl.normalizeText(verifyVariable_cafe_9_9 + verifyVariable_cafe_10_10),
		ztl.normalizeText("3"),
	);
});
