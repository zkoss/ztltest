...
fixture`ZTL TEST - B36-2918492TestCafe`
...
test("B36-2918492TestCafe", async (t) => {
...
<portallayout>
	...
	await t
		.wait(2000)
		.click(Selector(() => zk.Widget.$(jq("$googletools")).$n("max")));
	await ztl.waitResponse(t);
	await t
		.expect(await ClientFunction(() => jq("$labpixies").is(":visible"))())
		.notOk();
	await t.click(Selector(() => zk.Widget.$(jq("$googletools")).$n("close")));
	await ztl.waitResponse(t);
	await t
		.expect(await ClientFunction(() => jq("$labpixies").is(":visible"))())
		.ok();
