...
fixture`ZTL TEST - Z30-grid-0026TestCafe`
...
test("Z30-grid-0026TestCafe", async (t) => {
	...
	await t
		...
		.eql(
			ztl.normalizeText(
				await ClientFunction(() =>
					jq("@row:eq(0) @label:eq(0)").text().replace(/\s/g, " "),
				)(),
			),
		);
	await t.click(Selector(() => zk.Widget.$("$add").$n("real")));
	await t
		.expect(await ClientFunction(() => zk.Widget.$("$add").isChecked())())
		.ok();
	await t.click(Selector(() => jq("$btnStart")[0])).wait(3200);
	let amount_cafe = await ClientFunction(() => jq("@row").length)();
	await t
		.click(Selector(() => zk.Widget.$("$add").$n("real")))
		.click(Selector(() => zk.Widget.$("$remove").$n("real")));
	await t
		.expect(
			await ClientFunction(
				() => zk.Widget.$("$add").$n("real").checked,
			)(),
		)
		.notOk();
	await t
		.expect(
			await ClientFunction(
				() => zk.Widget.$("$remove").$n("real").checked,
			)(),
		)
		.ok();
	await t
	...
	await t.wait(3200).click(Selector(() => jq("$btnStop")[0]));
	await ztl.waitResponse(t);
	let newamount_cafe = await ClientFunction(() => jq("@row").length)();
	await t.expect(newamount_cafe < amount_cafe).ok();
	await t.wait(1700);
	let verifyVariable_cafe_0_0 = await ClientFunction(
		() => jq("@row").length,
	)();
	await t.expect(newamount_cafe == verifyVariable_cafe_0_0).ok();
});
