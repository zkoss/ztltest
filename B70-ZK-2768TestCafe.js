...
fixture`ZTL TEST - B70-ZK-2768TestCafe`
...
test("B70-ZK-2768TestCafe", async (t) => {
...
<zk>
	...
	await t.click(Selector(() => jq(".z-toolbarbutton").eq(0)[0]));
	...
	await t
		...
		.notOk();
	await t.click(Selector(() => jq(".z-toolbarbutton").eq(1)[0]));
	await ztl.waitResponse(t);
	await t
		.expect(
			await ClientFunction(
		...
		.notOk();
	await t.click(Selector(() => jq(".z-toolbarbutton").eq(2)[0]));
	await ztl.waitResponse(t);
	await t
		.expect(
			await ClientFunction(
		...
		.notOk();
	await t.click(Selector(() => jq(".z-toolbarbutton").eq(3)[0]));
	await ztl.waitResponse(t);
	await t
		.expect(
			await ClientFunction(
