	...
	<vlayout width="100%" height="100%">
		...
		runZTL(zscript,
			() => {
				...
				scroll_outer = findElement(By.cssSelector(".z-listbox-body > div"));
				...
				waitResponse(2500, true);
				
				// should see item after 50
				touch.down(location.getX() + 10, location.getY() + 10);
				waitResponse(1000, true);
				
				var selected_item = listbody.findElement(jq(".z-listitem-seld .z-listcell-cnt")).getText().substring(7).toInt;
				...
				scroll_outer = findElement(By.cssSelector(".z-grid-body > div"));
				scroll_bar   = scroll_outer.findElement(By.cssSelector("div"));
				verifyEquals("0", scroll_outer.getCssValue("opacity"));
