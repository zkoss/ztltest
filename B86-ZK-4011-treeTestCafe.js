...
fixture`ZTL TEST - B86-ZK-4011-treeTestCafe`
	.page`http://localhost:8080/zktest/test2/B86-ZK-4011-tree.zul`.beforeEach(
	async () => {
...
test("B86-ZK-4011-treeTestCafe", async (t) => {
	...
	await ztl.verifyTolerant(
		...
		ztl.normalizeText("0"),
		ztl.normalizeText(
			await ClientFunction(() => jq("@treecell:eq(0)")[0].clientWidth)(),
		),
		ztl.normalizeText("1"),
	);
	await ztl.verifyTolerant(
		...
		ztl.normalizeText("0"),
		ztl.normalizeText(
			await ClientFunction(() => jq("@treecell:eq(3)")[0].clientWidth)(),
		),
		ztl.normalizeText("1"),
	);
	await ztl.verifyTolerant(
		...
		ztl.normalizeText("0"),
		ztl.normalizeText(
			await ClientFunction(() => jq("@treecell:eq(5)")[0].clientWidth)(),
		),
		ztl.normalizeText("1"),
	);
	await ztl.verifyTolerant(
